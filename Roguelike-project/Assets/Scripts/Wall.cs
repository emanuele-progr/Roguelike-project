    ...
    public void DamageWall (int loss, string dealer)
    {
        ...
        if (hp <= 0)
            ...
            else if (!opened)
            {
                ...
                if (dealer == "player")
                {
                    GameManager.instance.chestOpened += 1;
                    if (randomLoot == 0)
                    {
                    ...
                    {
                        if (!GameManager.instance.hasHealthPotion)
                        {
                            lootSprite.sprite = lootPool[2];
                            //lootPool.Remove(lootPool[2]);
                            //GameManager.instance.inventoryIcons = lootPool;
                            GameManager.instance.hasHealthPotion = true;
                        }
                        else
                        {
                            StartCoroutine(disappearObject());
                    ...
                    {
                        if (!GameManager.instance.hasSpeedPotion)
                        {
                            lootSprite.sprite = lootPool[3];
                            //lootPool.Remove(lootPool[2]);
                            //GameManager.instance.inventoryIcons = lootPool;
                            GameManager.instance.hasSpeedPotion = true;
                        }
                        else
                        {
                            StartCoroutine(disappearObject());
